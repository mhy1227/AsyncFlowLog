...
- 归档后迁移原始：将源 `*.log` 移动到 `async.log.archive.raw-dir`（保留原始文本，便于审计与对比）。
...
## 附：重名规避说明
- 当归档目录已存在同名 zip 或 raw 目录已存在同名 .log 时，系统会在目标文件名中追加时间戳（`yyyyMMdd-HHmmss`）后再落盘/迁移，避免覆盖。
- 例：
  - `async-log-2025-10-23.log.zip` → `async-log-2025-10-23.log.20251024-070501.zip`
  - `async-log-2025-10-23.log`     → `async-log-2025-10-23.20251024-070501.log`
