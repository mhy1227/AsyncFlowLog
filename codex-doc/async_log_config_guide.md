...
### 5. 业务代码使用示例\n1. **引入依赖与配置**：
   - 确保当前项目的 Maven `settings.xml` 或 `<repositories>` 能访问 AsyncFlowLog 所在的私服；
   - 在 `pom.xml` 中加入 `<dependency>`（见上文章节）；
   - 在 `application.yml` 中补充 `async.log.*`、`spring.datasource.*`、`logging.*` 等配置。

2. **注入并调用 `AsyncLogService`**（显式日志）：
   ```java
   @Autowired
   ...
   ```
   - 业务线程会快速返回，日志消息被放入队列，由后台线程写入配置的存储。

3. **使用 `@OperationLog` 注解**（可选）：
   - 在业务方法上添加注解，切面会自动采集参数、返回值、耗时等信息并异步记录；
   - 在生产环境下建议配合操作日志表（参见数据库脚本 `db/init.sql`）。

4. **验证集成**：
   - 启动应用后访问 `/actuator/health`，确认 `asynclog` 状态为 `UP`；
   - 触发一次日志事件，检查 `logs/asyncflow.log` 或 MQ/数据库是否写入；
   - 通过指标或日志观察队列长度、线程池活跃数，确保与预期一致。

5. **常见问题**：
   - **日志文件为空**：业务代码尚未调用 `AsyncLogService` 或注解未触发；
   - **数据库连接失败**：确认 JDBC URL、账号、权限，以及网络连通性；
   - **队列告警**：调整 `queue.capacity`、消费者线程数，或更换写入器。

1. **配置**：在业务项目中覆盖 `async.log.*` 参数，例如：
   ...
   async:
     log:
       ...
       appender:
         type: file
         file-path: /var/log/async-demo
   ```

2. **直接调用 `AsyncLogService`**：
   ...
   public class OrderController {
       ...
       public ResponseEntity<String> create(@RequestBody OrderCreateDTO dto) {
           ...
           asyncLogService.log("INFO", "订单创建成功", context);
           return ResponseEntity.ok("ok");
       }
   }
   ```

3. **使用 AOP 操作日志**（可选）：在方法上添加 `@OperationLog` 注解，AsyncFlowLog 会通过切面记录操作并异步写入日志。

4. **与现有日志框架共存**：常规业务日志仍可通过 `Slf4j` 输出；AsyncFlowLog 负责结构化、可扩展的异步日志，两者互不冲突。
